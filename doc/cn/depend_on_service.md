# 响应式服务 (`MvcDependableObject`)

除了使用 `MvcController` 和 `MvcRawStore` 进行状态管理外，`flutter_mvc` 还提供了一种更轻量、更灵活的方式来让 Widget 响应变化：依赖于一个可观察的对象。

你可以让任何 Dart 类变得“可依赖”，只要它混入 (`mixin`) `MvcDependableObject`。当这个对象的状态发生变化时，它可以通知所有依赖于它的 Widget 进行重建。
...
2.  **`context.dependOnObject(object)`**:
    这是一个 `BuildContext` 的扩展方法。当你在 Widget 的 `build` 方法中调用它时，它会将当前 Widget 注册为 `object` 实例的一个“依赖者”。

当服务实例调用 `notifyAllDependents()` 时，所有注册的依赖者（即那些调用了 `dependOnObject` 的 Widget）都会被标记为“需要重建”，从而实现 UI 的自动更新。
    ...
    MaterialApp(
      home: MvcApp(
        child: MvcDependencyProvider(
          ...
          child: Scaffold(
            body: const Center(
              child: CounterText(), // 使用独立的 Widget
            ),

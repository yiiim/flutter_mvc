    ...
    MaterialApp(
      home: MvcApp( // MvcApp 是所有 flutter_mvc 应用的根
        child: Mvc(
          ...
          model: IndexPageModel(),        // 可选：传入初始数据模型
        ),
      ),
    ),
  );
...
-   `create`: 一个返回 `MvcController` 实例的函数。这是必须的，除非你通过[依赖注入](./dependency_injection.md)在父级作用域中提供了 Controller。
-   `model`: 一个可选的数据模型对象，它将被传递给 Controller。如果你的 Controller 不需要从外部接收初始数据，可以省略此参数。
...
class IndexController extends MvcController<IndexPageModel> {
  ...
  @override
  void didUpdateModel(IndexPageModel oldModel) {
    super.didUpdateModel(oldModel);
    // 当 Mvc Widget 的 model 属性被外部更新时调用
    if (oldModel.title != model.title) {
      print("Model has been updated externally.");
    }
  }
}
```

-   **泛型**: `MvcController<TModelType>` 中的 `TModelType` 是关联的 Model 类型。如果没有 Model，可以使用 `void`，即 `MvcController<void>`。
-   **`view()`**: 必须实现此方法，返回一个 `MvcView` 实例。
-   **`model`**: 通过 `model` 属性可以访问传递给 `Mvc` Widget 的数据模型。

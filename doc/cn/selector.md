...
通过为 `MvcWidget` 设置 `className` 属性（可以是一个或多个类，用空格分隔），你可以使用类选择器来查找一组控件。

**示例**:

```dart
// 定义带 class 的控件
MyWidget(className: 'card highlight');
MyWidget(className: 'card');
...
你可以根据 `MvcWidget` 的任意公开属性来进行查找，但该属性必须使用 `@Attribute()` 进行注解。
...
MyWidget(status: 'inactive');

// 查找所有 status 为 'active' 的控件
final activeWidgets = querySelectorAll<MyWidget>('[status=active]');
```
...
-   **相邻兄弟选择器 (`+`)**: `A + B` - 查找紧跟在 `A` 控件后面的同级 `B` 控件。
-   **后续兄弟选择器 (`~`)**: `A ~ B` - 查找在 `A` 控件后面的所有同级 `B` 控件。
...
// 查找 class 为 'header' 的控件的下一个同级 'content' 控件
final content = querySelector('.header + .content');
```

## 性能提示

选择器查询会遍历控件树的相关部分，因此应避免在 `build` 方法中频繁调用。最佳实践是在事件处理、生命周期方法（如 `initState`）或需要时执行查询。

查询结果 (`MvcWidgetScope` 或 `List<MvcWidgetScope>`) 可以被缓存和复用，直到相关控件从树中移除。

...
`flutter_mvc` 提供了一个简单的状态管理解决方案，称为 Store。Store 是一个可以存储状态的对象，并且它还可以关联一个 `BuildContext`，以便在状态变化时通知相关的 Widget 进行重建。
...
> 得益于状态独立作用域，即使在子作用域中创建的状态，只要位于同一个状态作用域，父作用域也可以访问它（这里的作用域指的是服务作用域）。

...
///
/// 可以一次性提供多个只有逻辑的控制器
class MvcMultiProxy<TProxyControllerType extends MvcProxyController> extends StatelessWidget {
  const MvcMultiProxy({Key? key, required this.child, required this.proxyCreater}) : super(key: key);
  final Widget child;
  final List<TProxyControllerType Function()> proxyCreater;
  @override
...
///
/// 可以一次性提供多个只有逻辑的单例控制器
class MvcMultiSingleProxy<TProxyControllerType extends MvcProxyController> extends StatelessWidget {
  const MvcMultiSingleProxy({Key? key, required this.child, required this.singleProxyCreater}) : super(key: key);
  final Widget child;
  final List<TProxyControllerType Function()> singleProxyCreater;
  @override
  Widget build(BuildContext context) {
    Widget widget = child;
    for (var element in singleProxyCreater) {
      widget = MvcSingle(creater: element, model: widget);
    }
    return widget;
  }
}

/// Copy from: https://github.com/dart-lang/html/blob/main/lib/src/query_selector.dart
/// We don't need to depend on the entire html library.
...
class SelectorEvaluator extends Visitor {
  ...
  bool matches(MvcNode element, SelectorGroup selector) {
    _element = element;
  ...
  FormatException _unsupported(TreeNode selector) => FormatException("'$selector' is not a valid selector");
  ...
  bool visitPseudoClassSelector(PseudoClassSelector node) {
    switch (node.name) {
      ...
      case 'root':
        ...
        return _element!.localName == 'html' && _element!.parent == null;

      // http://dev.w3.org/csswg/selectors-4/#the-empty-pseudo
      case 'empty':
        return _element!.children.isEmpty;
    ...
    switch (node.name) {
      ...
      case 'nth-child':
        ...
        if (exprs.length == 1 && exprs[0] is LiteralTerm) {
          ...
          return parent != null && (literal.value as num) > 0 && parent.children.indexOf(_element!) == literal.value;
        }
        break;
  ...
  static String? _getInheritedLanguage(MvcNode? node) {
    while (node != null) {
  ...
  @override
  bool visitElementSelector(ElementSelector node) => node.isWildcard || _element!.localName == node.name.toLowerCase();

  @override
  bool visitIdSelector(IdSelector node) => _element!.id == node.name;

  @override
  bool visitClassSelector(ClassSelector node) => _element!.classes.contains(node.name);
  ...
  bool visitAttributeSelector(AttributeSelector node) {
    ...
    return switch (node.operatorKind) {
      ...
      TokenKind.INCLUDES => value.split(' ').any((v) => v.isNotEmpty && v == select),
      TokenKind.DASH_MATCH => value.startsWith(select) && (value.length == select.length || value[select.length] == '-'),
